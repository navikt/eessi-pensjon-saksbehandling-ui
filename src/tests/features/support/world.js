...
class World {
  constructor () {
    ...
    this.elementLoads = async function (cssPattern, parentEl) {
      await this.driver.wait(webdriver.until.elementLocated(webdriver.By.css(cssPattern, parentEl)))
    }

    this.getElement = async function (cssPattern, parentEl) {
      await this.driver.findElement(webdriver.By.css(cssPattern, parentEl))
    }

    this.getElements = async function (cssPattern, parentEl) {
      await this.driver.findElements(webdriver.By.css(cssPattern, parentEl))
    }

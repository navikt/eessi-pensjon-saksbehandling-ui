...
class StorageModal extends Component {
    ...
    componentDidUpdate() {
        ...
        const { t, userInfo, modalStorageOpen, fileList, fileLoaded, savingStorageFile, deletingStorageFile, modalStorageOptions, loadingStatus, loadingStorageFileList, actions } = this.props;
        ...
        if (modalStorageOpen) {
            ...
            if (!fileList && !loadingStorageFileList && loadingStatus !== 'ERROR') {
                  actions.listStorageFiles(userInfo);
            }
    ...
    onOkClick() {
        ...
        const { userInfo, actions, modalStorageOptions } = this.props;
        const { fileSelected, saveTargetFileName } = this.state;

        if (modalStorageOptions.action === 'open') {
            actions.getStorageFile(userInfo, fileSelected);
            this.setState({
                fileSelected : undefined
            });
        }
        if (modalStorageOptions.action === 'save') {
            ...
            actions.postStorageFile(userInfo, targetFile, modalStorageOptions.content);
        }
    }
    ...
    onDeleteFile(e) {
        ...
        e.preventDefault();
        const { userInfo, actions, fileToDelete } = this.props;

        actions.deleteStorageFile(userInfo, fileToDelete);
    }

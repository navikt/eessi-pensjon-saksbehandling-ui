...
import { LocalStorageEntry, Validation } from 'declarations/app'
...
const SendSEDModal = <CustomLocalStorageContent extends any = any>({
  ...
  const performValidation = (): boolean => {
    ...
    if (!_name) {
      validation['savesedmodal-name'] = {
        ...
        feilmelding: t('buc:validation-noName')
  ...
  const onSave = async () => {
    if (performValidation()) {
      ...
      const newSavedEntries = savedEntries!.concat({
        name: _name,
        date: dateString,
        content: localStorageContent
      } as LocalStorageEntry<CustomLocalStorageContent>)
      await window.localStorage.setItem(storageKey, JSON.stringify(newSavedEntries, null, 2))
      setSaved(true)
      setMessage(t('buc:p5000-saved-svarsed-draft', { name: _name, date: dateString }))
    }
  }

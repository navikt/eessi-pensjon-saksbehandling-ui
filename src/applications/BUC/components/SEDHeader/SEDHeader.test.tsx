...
describe('applications/BUC/components/SEDHeader/SEDHeader', () => {
  ...
  beforeEach(() => {
    ...
    wrapper = render(<SEDHeader {...initialMockProps} />)
  })
  ...
  it('Render: has proper HTML structure', () => {
    ...
    expect(wrapper.find('[data-testid=\'a_buc_c_sedheader--name-id').hostNodes().render().text()).toEqual('P2000 - buc:buc-P2000')
    expect(wrapper.find('[data-testid=\'a_buc_c_sedheader--status-id').render().text()).toEqual('buc:status-' + sed!.status)
    expect(wrapper.find('[data-testid=\'a_buc_c_sedheader--version-date-id').hostNodes().render().text()).toEqual('29.05.2019')
    expect(wrapper.find('[data-testid=\'a_buc_c_sedheader--version-id').hostNodes().render().text()).toEqual('ui:version: 5')
    expect(wrapper.find('[data-testid=\'a_buc_c_sedheader--institutions-id').first().render().text()).toEqual('NO:DEMONO02')
    expect(wrapper.find('[data-testid=\'a_buc_c_sedheader--institutions-id').last().render().text()).toEqual('NO:DEMONO01')

    const actions = wrapper.find('[data-testid=\'a_buc_c_sedheader--actions-id').hostNodes()
    expect(actions.exists('FilledPaperClipIcon')).toBeTruthy()

    expect(actions.exists('[data-testid=\'a_buc_c_sedheader--answer-button-id')).toBeTruthy()
  })

  it('Handling: handling answer button click', () => {
    (initialMockProps.onFollowUpSed as jest.Mock).mockReset()
    const replySedButton = wrapper.find('[data-testid=\'a_buc_c_sedheader--answer-button-id').hostNodes().first()

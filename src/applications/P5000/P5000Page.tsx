...
`
export interface P5000PageProps {
  buc: Buc
...
const P5000Page: React.FC<P5000PageProps> = ({
  buc,
  context,
  mainSed = undefined,
  setMode,
  slots
}: P5000PageProps): JSX.Element => {
  ...
  const renderTable = (label: string): JSX.Element => {
    if (label === 'P5000Edit') {
      return <Draggable draggableId='P5000Edit' index={1}>
        {(provided, snapshot) => (
          <div
    ...
    if (label === 'P5000Sum') {
      return <Draggable draggableId='P5000Sum' index={1}>
        {(provided, snapshot) => (
          <DraggableDiv
            className={classNames({isDragging: snapshot.isDragging})}
            ref={provided.innerRef}
    ...
    if (label === 'P5000Overview') {
      return  <Draggable draggableId='P5000Overview' index={1}>
        {(provided, snapshot) => (
          <div
            className={classNames({isDragging: snapshot.isDragging})}
            ref={provided.innerRef}

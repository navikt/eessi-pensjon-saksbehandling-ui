...
const bucReducer = (state: State = initialBucState, action: Action) => {
  switch (action.type) {
    ...
    case types.BUC_GET_BUC_LIST_SUCCESS:
      ...
      return {
        ...state,
        bucList: _.difference(action.payload, ['P_BUC_02', 'P_BUC_05', 'P_BUC_10'])
      }
    ...
    case types.BUC_GET_SED_LIST_SUCCESS: {
      ...
      return {
        ...state,
        sedList: action.payload.sort((a: string, b: string) => {
    ...
    case types.BUC_GET_INSTITUTION_LIST_SUCCESS: {
      ...
      action.payload.forEach((institution: RawInstitution) => {
